...
class Booking extends React.Component {
    constructor(props) {
        ...
        this.concession = this.concession.bind(this);
    ...
    payment(a){
        this.setState({tickets:<ModalFooter><Button color="secondary" onClick={() => this.adult(a)}>Adult</Button><Button color="secondary" onClick={() => this.child(a)}>Child</Button><Button color="secondary" onClick={() => this.concession(a)}>Concession</Button>{' '}</ModalFooter>});
    }

    adult(a){
        this.state.payment.push([a,"Adult",10.00]);
        this.setState({tickets:<ModalFooter><Button color="danger" onClick={this.finalPurchase}>Purchase Tickets</Button></ModalFooter>});
    }

    child(a){
        this.state.payment.push([a,"Child",4.00]);
        this.setState({tickets:<ModalFooter><Button color="danger" onClick={this.finalPurchase}>Purchase Tickets</Button></ModalFooter>});
    }

    concession(a){
        this.state.payment.push([a,"Concession",5.00]);
        this.setState({tickets:<ModalFooter><Button color="danger" onClick={this.finalPurchase}>Purchase Tickets</Button></ModalFooter>});
    }
